...
/* Max inflight ULT and data size per xstream */
#define MIGRATE_MAX_SIZE	(1 << 30)
#define MIGRATE_MAX_ULT		64
...
migrate_one_ult(void *arg)
{
	...
	if (data_size == 0) {
		D_DEBUG(DB_REBUILD, "skipe empty mrone %p\n", mrone);
		D_GOTO(out, rc);
	}
		     ...
		     d_iov_t *val_iov, void *data)
{
	...
	while (!dbtree_is_empty(root->root_hdl)) {
		...
		ult_cnt = min(tls->mpt_obj_generated_ult -
			      tls->mpt_obj_executed_ult,
			      tls->mpt_generated_ult -
			      tls->mpt_executed_ult);
